...
if(isset($_GET['id'])){
	...
	$cek = mysql_query("SELECT id_buku FROM buku WHERE id_buku='$id'") or die(mysql_error());
	...
	if(mysql_num_rows($cek) == 0){
		...
		$del = mysql_query("DELETE FROM buku WHERE id_buku='$id'");
		
		//jika query DELETE berhasil
		if($del){
			
			echo 'Data siswa berhasil di hapus! ';		//Pesan jika proses hapus berhasil
			echo '<a href="?p=buku/index-book">Kembali</a>';	//membuat Link untuk kembali ke halaman beranda
